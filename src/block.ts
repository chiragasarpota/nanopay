...
import { deriveAddress, derivePublicKey } from './keys.js'

import { unsafeHashBlock } from './hash.js'

import { signBlock } from './signature.js'
...
export function createBlock(secretKey: string, data: BlockData): Block {
  ...
  if (!data || typeof data !== 'object')
    throw new Error('Block data is not valid')
  if (typeof data.work === 'undefined') throw new Error('Work is not set')
  if (data.work !== null && !checkWork(data.work))
    throw new Error('Work is not valid')
  ...
  */
  ...
  let linkAsAddress
  if (linkIsAddress) {
    linkAsAddress = deriveAddress(derivePublicKey(correctedLink))
