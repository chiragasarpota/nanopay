...
| `getReceivable(address, { count = 100, offset = 0, signal? })` | Confirmed incoming `{ hash, amount, amountRaw, source }[]`. Offset is a non-negative safe integer for paging.                                                                                 |
...
| `getBlock(hash, options?)`                                     | `{ account, amount?, amountRaw?, balance, balanceRaw, confirmed, subtype?, block }`. Amount fields are absent when a pruned predecessor prevents calculation.                                 |
...
On a pruned node, a retained block can have an unavailable amount while its balance and confirmation status remain available. `getBlock` omits both amount fields in that case; malformed amounts still reject. `waitForConfirmation` can confirm such a block. If the requested block itself was pruned, `getBlock` and the node's hash-only `isReceivable` RPC report `Block not found`; account-scoped `getReceivable` records remain available.
...
| `resumeAccount(address, transactionHash)` | Clear the local account block after the caller reconciles that exact failed submission. Synchronous; throws if the account/hash does not match a blocked transaction.                                    |
...
`prepareReceive` and `receive` use retained source history when available. If the source or its predecessor was pruned, they find the confirmed receivable record for the destination account in pages of 1000 entries. This preserves ownership and amount checks without requiring historical block contents. The fallback supports cancellation and may require multiple requests on accounts with many receivables. Paging reads current records; coordinate concurrent receivers as described below.
...
A failed submission blocks the account, including failures wrapped in `ReceiveAllError`. Queued and future writes reject with `AccountBlockedError`; its `transaction` identifies the earlier candidate and `cause` is the original `TransactionError`. Reads and writes to other accounts continue. After reconciling the candidate with the node, call `resumeAccount(address, transactionHash)` to allow new writes. This method only clears local state; it does not check, cancel, retry, or confirm a submission. A missing hash alone is insufficient while the node may still be processing it. Previously queued writes remain rejected even if you resume immediately. Failures before publication do not block the account.
